...
/**
 * Cleans up all cursor entries with the specified ID.
 *
 * @param {HTMLElement} element - DOM element to manage.
 * @param {string} id - Cursor entry ID.
 */
const cleanupEntries = (element, id) => {
  if (!registry.has(element)) {
    return;
  }
...
class CursorClient {
  ...
  /**
   * Cleans up all cursor entries for this client.
   */
  cleanup() {
    cleanupEntries(this.element, this.id);
  }
